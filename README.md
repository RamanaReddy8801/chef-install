...
The `nrdot-collector-mssql*` and `nrdot-collector-oracle*` targets configure one collector against one or more instances via an instances file (YAML), rather than inline server/credential env vars. The SQL-auth variants (`nrdot-collector-mssql`, `nrdot-collector-mssql-rds`, `nrdot-collector-oracle*`) also take a secrets file (`KEY=VALUE` per instance); the Windows Domain Auth / gMSA variants (`nrdot-collector-mssql-winauth`, `nrdot-collector-mssql-rds-winauth`) use only the instances file (host/port per instance) since one Windows identity, set via env vars, monitors every instance. The exact `NR_CLI_*` env vars vary by variant (local vs RDS, SQL auth vs Windows/gMSA auth). No defaults are declared for these — set the vars relevant to your chosen target via `default['newrelic_install']['env'][...]`. See the comments in `attributes/default.rb` and the corresponding recipe definitions in [open-install-library](https://github.com/newrelic/open-install-library) for the exact list per target.
